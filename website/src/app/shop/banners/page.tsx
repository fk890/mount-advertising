...
export default function Banners() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('/api/products?category=banners');
      ...
      <div className="w-full py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-400 max-w-2xl mx-auto" style={{ fontFamily: "var(--font-montserrat)" }}>
            Eye-catching banner solutions for effective advertising.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <p className="text-xl text-gray-400 mb-4">No banner products found. Check back soon!</p>
          </div>
        )}
      </div>
    </div>
